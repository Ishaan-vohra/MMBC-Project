...
public:
   ...
   eventRates(std::string flux_directory, double pot_per_file, double user_position_x, double user_position_y, double user_position_z, std::string output_name);
...
private:
   std::ifstream fdat_file[36]; // CHANGE VALUES
   int fnbins;
   int fnlines;
   double f_e_arr[1500][36][2];    // energy bins; neutrino type; CC/NC
   double f_xsec_arr[1500][36][2]; // energy bins; neutrino type; CC/NC

   inline static std::string neutrino_types_array[6] = {"numu", "numubar", "nue", "nuebar", "nutau", "nutaubar"};
   inline static std::string currents_array[2] = {"cc", "nc"};
   inline static std::string processes_array[6] = {"coh", "dis", "mec", "qel", "res", "tot"};

   TH1D *fluxHists[6];
   TH1D *eventRateHists[6][2][6];
};
...
eventRates::eventRates(std::string flux_directory, double potperfile, double position_x, double position_y, double position_z, std::string output_name)
{
   ...
   ffilename = flux_directory + "/output_area/histos_" + output_name;
